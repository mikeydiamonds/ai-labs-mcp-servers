...
server.registerTool(
  ...
  {
    description:
      "Submit a single URL to Google's Indexing API. ONLY use on pages with JobPosting or BroadcastEvent schema — per Google's official policy the API is restricted to those content types, and for anything else Google silently ignores the submission (John Mueller has confirmed this publicly). Do NOT use to 'push' blog posts, glossary entries, landing pages, or comparison pages to Google — submit the sitemap instead. The one broadly safe use is URL_DELETED to speed up removal of a retired page that already returns 404/410. URL_UPDATED on unsupported content types has no indexing benefit and sustained misuse can revoke the org's Indexing API access. If you're not sure the page is a JobPosting or BroadcastEvent, do not call this tool.",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "Submit multiple URLs to Google's Indexing API. Same eligibility rules as submit_url: ONLY for pages with JobPosting or BroadcastEvent schema, or for URL_DELETED on retired pages. Google ignores URL_UPDATED submissions on other content types and may revoke access for sustained misuse. Shared 200 URL/day quota across the project — batch size counts against it. This is a sequential loop (not a true HTTP batch), so 200 URLs = 200 API calls. Do NOT use to bulk-submit blog posts, glossary pages, or marketing content; submit the sitemap instead.",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "Submit a sitemap to Google Search Console. This is the preferred way to prompt Google to discover and re-crawl standard content (blog posts, glossary entries, landing pages). Use this instead of submit_url/batch_submit for any page without JobPosting or BroadcastEvent schema. The sitemap must be accessible at the provided URL.",
    inputSchema: {
      siteUrl: siteUrlSchema,
